...
int main(int argc, char *argv[])
{
    ...
    // 车代码等相关文件迁移
    splash.showMessage("车代码相关初始化...", Qt::AlignBottom|Qt::AlignLeft, Qt::red);
    if(!myFile.isDirectoryExist("Popkart", myFile.pathDocument())) {
        myFile.createDirectory("Popkart", myFile.pathDocument());
        myFile.writeTextFile("0", "carCodeVersion.txt", myFile.pathDocument() + "/Popkart");
    }
    QStringList dirList = {"2.0", "3.0", "2.1", "Park", "进化", "X"};
    for(QString dir : dirList) {
        if(!myFile.isDirectoryExist(dir, myFile.pathDocument() + "/Popkart")) {
            myFile.createDirectory(dir, myFile.pathDocument() + "/Popkart");
        }
        QStringList fileList = myFile.fileInDirectory("./" + dir, true);
        for(QString file : fileList) {
            if(!myFile.isFileExist(file, myFile.pathDocument() + "/Popkart/" + dir))
                myFile.copyFile(myFile.pathDocument() + "/Popkart/" + dir, file, "./" + dir);
        }
    }
